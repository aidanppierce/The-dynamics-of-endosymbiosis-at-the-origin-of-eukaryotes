...
def selection(matrix,s,theta): #matrix,Se,Se_neg,Se_plus,Se1,Sh,Sh_neg,Sh_plus,Sh1,kappa,Hg,Ng
    ...
    wh = np.prod(1-(s*(np.sum(wh_matrix, axis = 1)/Ne)**theta), axis = 1)*np.prod(1+(s*(np.sum(matrix[:,:,indices[2]:indices[3]], axis = 1)/Ne)**theta), axis = 1)
...
def model_v10(matrix,U,s,theta,generations, repeat): #model_v9(matrix, u,Ng,Se,Se_neg,Se_plus,Se1,Sh,Sh_neg,Sh_plus,Sh1,kappa,Hg,generations, test, repeat):
    np.save(f'v10_s{s}_theta{theta}_START_{repeat}_matrix', matrix)
    for n in range(0,generations):
        ...
        if n%10000 == 0:
            print(n)
            #np.save(f'v10_s{s}_theta{theta}_{n}_{repeat}_matrix', matrix)
    np.save(f'v10_s{s}_theta{theta}_100000_{repeat}_matrix', matrix)
    return matrix


Nh_list = [100]#np.arange(100,0,-5)#[1,10,100,1000]
Ne_list = [100]#np.arange(100,0,-5)#[100]
theta_list = [1]
s_list = [0.05]
Ng = 120
u = 0.0005
    ...
    for Ne in Ne_list:
        ...
        for theta in theta_list:
            for s in s_list:
                ...
                parameters.to_csv(f'v10_s{s}_theta{theta}_PARAMETERS.txt')

                #files = [name for name in os.listdir('.') if os.path.isfile(name)]
                #repeat = sum(f'v10_Ne_{Ne}_Nh_{Nh}_s{s}_theta{theta}_START_' in i for i in files)

                files = [name for name in os.listdir('.') if os.path.isfile(name)]
                #print(files)
                
                repeat = sum(f'v10_s{s}_theta{theta}_START_' in i for i in files)
                ...
                #for r in range(0,repeats):
                    #print(r)
                    #if os.path.isfile(f'{path}/v10_Ne_{Ne}_Nh_{Nh}_s{s}_theta{theta}_END_{r}_matrix.npy') == True:
                        #continue
                    #else:
                        #print(f'missing: v10_Ne_{Ne}_Nh_{Nh}_s{s}_theta{theta}_END_{r}_matrix.npy')
                #        #while repeat < repeats:
